...
class FolderRepository {
  ...
  async throwIfFolderNotFoundOrIsRoot(
    folder,
    userId,
    message = "Ошибка взаимодействия"
  ) {
    const rootFolder = await this.getRootFolder(userId);

    if (folder.id === rootFolder.id) {
      throw AppException.new({ code: StatusCode.BAD_REQUEST, message });
    }
  }
  ...
  async getRootFolder(userId) {
    try {
      return this.folderModel.findOne({ userId, parentId: null });
    } catch (e) {
      throw AppException.new({
  ...
  async getFolderByIdWithChildren(id, req) {
    try {
      ...
      const folderChildren = await this.folderModel.find({
        parentId: folder.id,
      });
      ...
      return {
        ...
        children: [
          ...folderChildren.map((folder) =>
            this.#pipeEntityType("folder", folder)
          ),
  ...
  async updateFolder(id, parentId, name, userId) {
    try {
      ...
      await this.throwIfFolderNotFoundOrIsRoot(
        folder,
        userId,
        "Папка запрещена для редактирования"
      );
      const body = {};
  ...
  async removeFolder(id, userId) {
    try {
      ...
      await this.throwIfFolderNotFoundOrIsRoot(
        folder,
        userId,
        "Папка запрещена для удаления"
      );

      const ids = [
        id,
        ...(await this.folderModel.find({ parentId: id })).map(
          (item) => item.id
        ),
      ];
